    ...
    <div className="rounded-xl border border-[#27272a] bg-[#18181b] p-6 shadow-xl animate-fade-in-up">
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        ...
        <div className="rounded-lg border border-[#27272a] bg-[#121214] px-4 py-3 shrink-0 flex items-center gap-4">
          <div>
            <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider">Weekly Momentum</p>
            <p className="mt-1 text-2xl font-bold text-zinc-100">{weeklyMomentum}%</p>
          </div>
          <div className="h-8 w-px bg-[#27272a]" />
          <div className="text-right">
            <p className="text-[10px] text-zinc-500 font-medium uppercase">Active Days Target</p>
            <p className="text-xs font-bold text-amber-500 mt-1">5 days / week</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6">
        <div className="flex flex-col gap-2 text-sm text-zinc-400 sm:flex-row sm:items-center sm:justify-between">
          <p>
            <span className="font-bold text-zinc-100">{currentXP.toLocaleString()} XP</span> earned
          </p>
          {!isMaxLevel ? (
            <p>
              <span className="font-bold text-zinc-100">{remainingXP.toLocaleString()} XP</span> to Level {level + 1}
            </p>
          ) : (
            <p className="text-amber-500 font-semibold">Ultimate Rank Achieved</p>
          )}
          {!isMaxLevel && (
            <p>
              Target: <span className="font-bold text-zinc-100">{nextLevelXP.toLocaleString()} XP</span>
            </p>
          )}
        </div>
        ...
        <div className="mt-3 relative">
          <div
            ...
            aria-valuetext={`${progressPercentage}% progress to next level`}
          >
            <div
      ...
      <div className="mt-6 border-t border-[#27272a] pt-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              {showActivity ? "[Hide XP Log]" : "[Show XP Log]"}
            </button>
          </div>
          <button
            type="button"
          ...
          >
            {showRules ? "Hide Rules" : "How XP Works"} &rarr;
          </button>
        </div>
        ...
        {showActivity && (
          recentActivity.length > 0 ? (
            ...
            <p className="mt-3 text-xs leading-6 text-zinc-550 italic">
              No local activity yet. Complete tasks or log workouts to start building momentum.
            </p>
          )
        )}
      </div>
