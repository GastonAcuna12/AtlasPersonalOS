...
export function TaskCard({
  ...
  const visibleSubtasks = isExpanded ? (task.subtasks ?? []) : (task.subtasks ?? []).slice(0, 3);
  ...
  return (
    <article className="rounded-xl border border-[#27272a] bg-[#18181b] p-5 shadow-lg flex flex-col justify-between hover:border-zinc-500 transition-all duration-300">
      <div>
        ...
        {/* Subtasks Section */}
        {((totalSubtasks > 0) || onUpdate) && (
          <div className="mt-4 border-t border-[#27272a]/40 pt-3.5">
            <div className="flex items-center justify-between mb-2.5">
              <p className="text-[10px] font-bold uppercase tracking-wider text-[#C8A96A]/95">
              ...
              <div className="flex flex-col">
                <ul className="space-y-2 mb-2">
                  {visibleSubtasks.map((sub) => (
                    <li key={sub.id} className="flex items-start gap-2.5 text-xs">
                      <input
                        type="checkbox"
                        ...
                        className={`mt-0.5 h-3.5 w-3.5 rounded focus:ring-0 focus:ring-offset-0 cursor-pointer disabled:cursor-not-allowed transition-all ${
                          sub.completed
                            ? "border-[#C8A96A] bg-[#C8A96A]/10 text-[#C8A96A]"
                            : "border-[#27272a] bg-[#121214] text-[#C8A96A]"
                        }`}
                      />
                      <span className={`break-all ${sub.completed ? "line-through text-zinc-500" : "text-zinc-300"}`}>
                        {sub.title}
                      </span>
                    </li>
                  ))}
                ...
                {totalSubtasks > 3 && (
                  <button
                    ...
                    className="text-[10px] font-bold text-[#C8A96A]/78 hover:text-[#C8A96A] transition uppercase tracking-wider mt-1 mb-2.5 text-left w-fit active:scale-95"
                  >
                    {isExpanded
                      ? (language === "es" ? "Mostrar menos" : "Show less")
                      : (language === "es" ? `Mostrar más (${totalSubtasks - 3})` : `Show more (${totalSubtasks - 3})`)}
                  </button>
                )}
              </div>
            )}
              ...
              totalSubtasks === 0 ? (
                !isAdding ? (
                  <button
                    ...
                    className="rounded-lg border border-[#27272a] bg-[#121214] hover:bg-[#18181b] hover:border-zinc-700 text-zinc-300 hover:text-white px-3.5 py-1.5 text-xs font-bold transition flex items-center gap-1.5 w-fit mt-1.5 active:scale-95"
                  >
                    ➕ {language === "es" ? "Agregar subtarea" : "Add subtask"}
                  </button>
                ) : (
                  <form onSubmit={handleAddSubtask} className="flex flex-col gap-2 mt-2">
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold text-zinc-400 hover:text-zinc-200 transition text-left self-start active:scale-95"
                    >
                ...
                <form onSubmit={handleAddSubtask} className="flex items-center gap-2 mt-2.5">
                  <input
                    type="text"
                    value={newSubtaskTitle}
                    onChange={(e) => setNewSubtaskTitle(e.target.value)}
